...
export async function getChannelInfo(Astro, { before = '', after = '', q = '', type = 'list', id = '' } = {}) {
  ...
  try {
    ...
    if (import.meta.env.DEV) {
      console.warn('Fetch failed (e.g. Telegram blocked), using mock data for dev:', err?.message)
      const mockChannelInfo = {
        posts: [{
          id: '1',
          title: '示例帖子（本地无法访问 Telegram 时显示）',
          type: 'text',
          datetime: new Date().toISOString(),
          tags: [],
          text: '这是一条占位内容。请配置代理或稍后在可访问 Telegram 的环境下运行以拉取真实频道。',
          content: '<p>这是一条<strong>占位内容</strong>。请配置代理或稍后在可访问 Telegram 的环境下运行以拉取真实频道。</p>',
        }],
        title: channel ? `${channel} (Mock)` : 'Channel (Mock)',
        description: '开发环境占位',
        descriptionHTML: '<p>开发环境占位</p>',
        avatar: undefined,
      }
      cache.set(cacheKey, mockChannelInfo)
